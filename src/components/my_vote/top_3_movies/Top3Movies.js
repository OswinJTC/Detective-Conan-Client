...
const Top3Movies = ({ movies }) => {
  ...
  const [vote2, setVote2] = useState([]);//取得 vote 物件
  ...
  const getVote2 = async (topicId) => {
    try {
      const response = await api.get(`/api/v1/votes/${topicId}`);
      setVote2(response.data);
    } catch (error) {
      console.error("Error fetching vote 2:", error);
    }
  };
  ...
  const arrayWithIndexes = vote2 && vote2.points ? vote2.points.map((value, index) => ({ value, index })) : [];
  ...
  const handleSelect = (index) => {
    ...
    if (selected.length < 3 || selected.includes(index)) {
    ...
    try {
      ...
      if (selected.length !== 3
       ...
       await api.put("/api/v1/votes", {
        ...
        correspondingPoints: selected.map((selectedIndex) => 3 - selected.indexOf(selectedIndex)),
        topicId: "2",
      });
...
<Row style={{ marginBottom: "20px", padding: "20px", borderBottom: "1px solid white" }}>
  <Col>
    <h2 className="text-center mt-3 mb-5" style={{fontSize: "100px"}}>最爛劇場版 Top 3</h2>
      ...
      <Container style={{ marginTop: "20px", textAlign: "center", color: "white"}}>
        ...
        <div>
         <Container style={{ margin: "0 auto", border: "1px solid white", padding: "10px" }}>
          ...
          : sortedArray.slice(0, 3).map((r, idx) => (
             <Container key={idx} style={{ border: "1px solid white", padding: "10px", background: "whitesmoke", margin: "10px" }}>
                <Row style={{ fontSize: "40px" }}>
                  <Col style={{ color: "black" }}>{idx + 1}</Col>
                  <Col style={{ paddingLeft: "20px", color: "black" }}>
        ...
        <Col md={3} className="vote-topics mb-5" style={{ borderRight: "1px solid white", height: "100%", paddingTop: "20px" }}>
          <div style={{ borderBottom: "1px solid white" }}>
            ...
            <ul className="list-group">
              ...
              <li className="list-group-item">
                <Link to="/PleaseWait">最懸疑</Link>
              </li>
              <li className="list-group-item">
                <Link to="/PleaseWait">最刺激</Link>
              </li>
              <li className="list-group-item">
                <Link to="/PleaseWait">最有藝術</Link>
              </li>
            </ul>
          </div>
          <div style={{ marginTop: "20px" }}>
            ...
            <ul className="list-group">
              <li className="list-group-item">
                <Link to="/PleaseWait">喜歡的角色</Link>
              </li>
              <li className="list-group-item">
                <Link to="/PleaseWait">討厭的角色</Link>
              </li>
              <li className="list-group-item">
                <Link to="/PleaseWait">最佳情侶黨</Link>
              </li>
            </ul>
          </div>
        </Col>
